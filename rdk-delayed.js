        ...
        trial(display_element, trial) {
            ...
            const startRDKDelayed = () => {
                for (let i = 0; i < trial.num_dots; i++) {
                    ...
                    dot.dx = vel.x + (Math.random() * 2 -1) * trial.dot_speed / 5
                    dot.dy = vel.y + (Math.random() * 2 -1) * trial.dot_speed / 5
                    dots.push(dot);
                }
                ...
                setTimeout(() => {
                    ...
                    for (let i = 0; i < trial.num_dots * trial.motion_coherence / 100; i++) {
                        if (trial.motion_direction === 'up') {
                            dots[i].dy = -trial.dot_speed + (Math.random() * 2 -1) * trial.dot_speed / 5
                            dots[i].dx = 0
                        } else {
                            dots[i].dy = trial.dot_speed + (Math.random() * 2 -1) * trial.dot_speed / 5
                            dots[i].dx = 0
